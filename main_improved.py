...
def create_qnn(model_accurate, quantization_threshold, quantized_value, std, mean, n_values, device):
    ...
    add_noise_to_weights(model_qnn, original=model_accurate, std=std, mean=mean, device=device)
    return model_qnn
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        model_qnn = create_qnn(model, quantization_threshold=args.quantization_threshold,
                               ...
                               n_values=args.n_values, device=device)
